...
// CabinEnforcer 获取adapter单例对象
func CabinEnforcer(ctx context.Context) (enforcer *casbin.SyncedEnforcer, err error) {
	once.Do(func() {
...
// newAdapter 初始化adapter操作
func (s *cabin) newAdapter(ctx context.Context) (a *adapterCasbin) {
	a = new(adapterCasbin)
	a.initPolicy(ctx)
	a.ctx = ctx
	return
